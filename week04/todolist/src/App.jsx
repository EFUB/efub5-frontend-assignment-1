...
const onCreate = (text) => { //새로운 할 일 만들 때 실행할 함수 
  const newTodo = {
    id: nextId.current,
    text, //사용자가 입력한 문자열 
    done:false,
  }
  setTodos([...todos, newTodo]) //기존 배열 복사 후 새 항목 붙임 
  nextId.current += 1;
}

const onRemove = (id) => {  //할일 삭제 함수 
  setTodos(todos.filter(todo=>todo.id !== id)); //삭제하려는 id 제외한 것만 남김 
}

const onToggle = (id) => {  //할일 완료 토글 기능 함수 
  setTodos(
    todos.map(todo =>
...
const onUpdate = (id, newText) => { //할일 수정 업데이트 함수 
  setTodos(
    todos.map(todo => 
